...
- XP를 구동시키는 원리는 상식적인 원리와 경험을 최대한 끌어 올리는 것이다.
...
- 여러 형태를 가지고 있따는 의미로, 여러 형태를 받아들일 수 있는 특징을 말한다.
- 메소드 오버라이딩은 상위 클래스에서 정의한 일반 메소드의 구현을 하위 클래스에서 무시하고 재정의할 수 있다.
- 오버로딩은 한 클래스 내에서 메서드 이름은 동일하지만 매개변수나 수나 타입을 다르게 하여 재정의하는것
...
유스케이스 다이어그램
- 유스케이스는 사용자 측면에서의 요구사랑으로, 사용자가 원하는 목표를 달성하기 위해 수행할 내용을 기술한다.
...
- 제어는 모델에 명령을 보냄으로써 모델의 상태를 변경할 수 있따.
...
- 실업무를 가지고 사용자가 직접 시험, 제어되지 않은 상태에서 행해진다.
- 발견된 오류와 사용상의 문제점을 주기적으로 개발자에게 보고한다.

하향식 통합 검사
: 상위 모듈에서 하위 모듈 방향으로 통합하여 검사
- 주요 제어 모듈 기준으로 아래로 통합하여 이동한다.
- 우선 통합법, 깊이 우선 통합법, 넓이 우선 통합법 등이 있다.

절차
1. 주요 모듈을 드라이버로 사용하고 주요 제어 모듈의 종속 모듈들은 스터브로 대체한다.
2. 깊이 우선, 넓이 우선 방식에 따라 종속 스터브들이 실제 모듈로 교체한다.
3. 모듈이 통합될 때마다 검사를 실시한다.
4. 새로운 오류가 생기지 않음을 보증하기 위해 회귀 검사를 실시한다.
- 드라이버 : 검사 자료 입출력 제어 프로그램
- 스터브 : 임시 제공되는 가짜 모듈, 모듈 간의 통합 시험을 위해 일시적으로 필요한 조건만을 가지고 임시로 제공되는 시험용 모듈

1. 브룩스(Brooks)의 법칙 : S/W 프로젝트 일정이 지연된다고 해서 프로젝트 말기에 새로운 인원을 추가 투입하면 프로젝트는 더욱 지연되게 된다고 주장하는 법칙
2. 요르돈 법칙 : SW 개발 초기 체계적인 분석 및 설계가 수행되지 않는다면 그 결과가 프로젝트 후반에 영향을 미치게되어 비용이 커진다 = 스노우볼
3. 파레토 법칙 : 전체 결과 80%가 전체원인의 20%에서 일어나는 현상을 가진다
4. 롱테일 법칙 : 사소해보이는 80%의 다수가 20% 소수 핵심보다도 뛰어난 가치를 창출해낸다

디지털 저작권 관리
: 저작권자가 배포한 디지털 컨텐츠가 저작권자의 의도한 용도로만 사용되도록 디지털 콘텐츠의 생성, 유통, 이용까지의 전 과정에 걸쳐 사용되는 디지털 컨텐츠 관리 및 보호 기술이다.

디지털 저작권 관리 기술 요소
- 암호화 : 콘텐츠 밑 라이선스를 암호화하고 전자 서명을 할 수 있는 기술
- 키 관리 : 콘텐츠를 암호화한 키에 대한 저장 및 분배 기술
- 암호화 파일 생성(패키저) : 콘텐츠를 암호화된 콘텐츠로 생성하기 위한 기술
...
3. FitNesse : 웹 기반 테스트케이스 설계, 실행, 결과 확인 등을 지원
4. NTAF : NHN(Naver)의 테스트 자동화 프레임워크(FitNesse + STAF 장점을 통합)
5. Selenium : 웹 애플리케이션
6. watir : 루비를 사용하는 앱
...
- 형상관리는 소프트웨어 개발에 모든 단계 및 유지보수 단계에서도 수행된다.
- 개발 전체 비용을 줄이고, 여러 방해요인을 최소화하는 것을 목적으로
- 관리항목에는 소스 코드 뿐만 아니라 각종 정의서, 지침서, 분석서 등이 포함된다.
- 소프트웨어에 가해지는 변경을 제거하고 관리한다.
- 유지보수 단계뿐만 아니라 개발 단계에도 적용할 수 있따.


화이트박스 테스트(White Box Test)
: 모듈 원시 코드를 오픈시킨 상태에서 **원시 코드의 논리적인 모든 경로**를 테스트하여 테스트 케이스를 설계하는 방법
- 설계된 절차에 초점을 둔 구조적 테스트이며, 테스트 과정의 초기에 적용된다.
- 모듈 안의 작동을 직접 관찰한다.
- 원시 코드(모듈)의 모든 문장을 한 번 이상 실행함으로써 수행된다.
- 프로그램의 제어 구조에 따라 선택, 반복, 등의 분시점 부분들을 수행함으로써 논리적 경로를 제어한다.
- 모듈의 논리적인 구조를 체계적으로 점검할 수 잇따.
- 테스트 데이터를 이용해 실제 프로그램을 실행함으로써 오류를 찾는 동적 테스트에 해당한다

화이트박스 테스트의 종류 : 조건, 루프, 데이터!
1. 기초 경로 검사
- 절차적 설계의 논리적 복잡성을 측정할 수 있게 해주는 테스트 기법
2. 제어 구조 검사
- 조건 검사 : 프로그램 모듈 내에 있는 *논리적 조건*을 테스트하는 테스트 케이스 설계 기법
- 루프 검사 : 프로그램의 *반복 구조*에 초점을 맞춰 실시하는 테스트케이스 설계 기법
- 데이터 흐름 검사 : 프로그램에서 *변수의 정의와 변수 사용의 위치*에 초점을 맞춰 실시하는 테스트 케이스 설계 기법

블랙박스 테스트
: 블랙박스 테스트는 소프트웨어가 수행할 *특정 기능*을 알기 위해서 각 기능이 완전히 작동되는 것을 입증하는 테스트로, 기능 테스트라고도 한다.
...
- 경계값 분석 : 동치 분할 기법을 보완하기 위한 기법, 입력 조건의 경계값을 테스트 케이스로 선정하여 검증
- 원인-효과 그래프 검사 : 입력 데이터 간의 관계와 출력에 영향을 미치는 상황을 체계적으로 분석한 다음 효용성이 높은 테스트 케이스를 선정하여 검사하는 기법이다.
- 오류 예측 검사 : 과거의 경험이나 확인자의 감각으로 테스트하는 기법, 데이터 확인 검사라고도 한다.
- 비교 검사 : 동일한 테스트 자료를 제공하여 동일한 결과가 출력되는지 테스트하는 기법이다.

외계인 코드(Alien Code)
: 아주 오래되거나 참고 문서 또는 개발자가 없어 유지보수 작업이 어려운 프로그램을 의미한다.

트리 
: 그래프의 특수한 형태로 노드(Node)와 선분(Branch)으로 되어 있고, 정점 사기에 사이클(Cycle)이 형성되어 있지 않으며, 자료 사이의 관계성이 계층 형식으로 나타나는 비선형 구조

전위순회 : 뿌리부터 방문
중위순회(Inorder Traversal) : 왼쪽부터 -> 뿌리 -> 오른쪽
후휘순위 : 왼 -> 오 -> 뿌
...
1. 삽입정렬 : 카드 끼우기 방식
2. 버블정렬
3. 선택정렬 : 리스트 중 최소값을 찾고 맨 앞 위치와 값을 교환
4. 퀵정렬 : 가운데 피벗 기준으로 버킷을 분해하고 버킷의 순서대로 레코드를 꺼내여 정렬
5. 힙정렬 : 최대 힙 트리나 최소 힙 트리를 구성해 정렬을 하는 방법
   - 정렬할 입력 레코드들로 힘을 구성하고, 가장 큰 키 값을 갖는 루트 노드를 제거하는 과정을 반복하여 정렬하는 기법
   - 평균 수행 시간은 O(nlog2n)이다
   - 완전 이진트리로 입력자료의 레코드를 구성한다.

스키마
- 내부스키마 : 물리적 저장 장치의 입장에서 본 데이터베이스 구조로서 실제로 데이터베이스에 저장될 레코드의 형식을 정의하고 저장 데이터 항목의 표현 방법, 내부 레코드의 물리적 순서 등을 나타낸다.
- 외부 스키마
- 개념 스키마
- 슈퍼 스키마

테스트 오라클 
- 테스트 결과가 참인지 거짓인지를 판단하기 위해서 사전에 정의된 참값을 입력하여 비교하는 기법 및 활동을 말한다.
- 종류에는 참, 샘플링, 휴리스틱, 일관성 검사가 존재한다.

빌드 자동화 도구의 개념
: 빌드 : 소스 코드 파일을 컴파일한 후 여러 개의 모듈을 묶어 실행 파일로 만드는 과정
...
해싱
: 아파트 우편물
![image](https://github.com/pointmina/EIP/assets/68779817/a39f8647-6f2a-489b-bfc9-bf0dee615ec0)
- 해싱은 직접 접근 파일을 구성할 때 사용되며, 접근 속도는 빠르나 기억공간이 많이 요구된다.
- 다른 방식에 비해 검색 속도가 가장 빠르다
- 삽입, 삭제 작업의 빈도가 많을 때 유리한 방식이다.
- 키 주소 변환 방법
- 버킷 : 하나의 주소를 갖는 파일의 한 구역, 버킷의 크기는 레코드 수를 의미한다.
- 슬롯 : 한개의 레코드를 저장할 수 있는 공간
- 충돌 : 서로 다른 두개 이상의 레코드가 같은 주소를 갖는 현상

해싱함수
1. 제산법
2. 제곱법
3. 폴딩법 : 레코드 키 값을 여러 부분으로 나눈 후 각 부분의 값을 더하거나 XOR 한 값을 홈 주소로 삼는 방식
4. 기수 변환법
5. 대수법 코딩법
6. 계수(숫자) 분석법
7. 무작위법
...
- 유용성 Usability : 쉽게 사용할 수 있는 정도
- 유연성 Flexibility : 새로운 요구사항에 맞게 얼마만큼 쉽게 수정할 수 있는지의 정도
- 재사용성
- 상호 운용성 Interperability : 다른 소프트웨어와 정보를 교환할 수 있는 정도

소프트웨어 형상관리
: 소프트웨어 생명주기 및 유지보수 과정에서 만들어지는 각 단계의 산출물을 체계적으로 관리하여 소프트웨어에 가시성, 추적성을 부여하여 품질보증활동을 향상시키는 기법
- 소프트웨어에서 일어나는 수정이나 변경을 알아냐고 제어하는 것을 의미한다
- 소프트웨어 개발의 전체 비용을 줄이고, 개발 과정의 여러 방해 요인이 최소화 되도록 보증하는 것을 목적으로 한다.
- 형상관리의 기능중 하나는 버전 제어 기술이다.
...
- 트리, 그래프 와 같은 계층적 구조를 나타냄

소프트웨어 통합 테스트
: 통합 테스트를 통해 각각의 소프트웨어 모듈들이 논리적으로 통합되고 그룹으로 테스트
- 단위 머듈과 프로그램이 이루는 그룹이 본 목적에 맞도록 동작되는지 확인

1. 상향식 테스트 : 하위 모듈부터 시작하여 상위 모듈로 테스트를 진행(테스트 드라이버)
2. 하향식 테스트 : 상위 모듈부터 하위 모듈로 테스트 진행(테스트 스탑)
3. 샌드위치 테스트 : 상향식과 하향식의 장점을 이용(상향 + 하향)

검증 VS 확인

![image](https://github.com/pointmina/EIP/assets/68779817/9e562828-5535-4af2-8a18-348a392152d7)

통합 테스트(Integration Test)
- 시스템을 구성하는 모듈의 인터페이스와 결합을 테스트하는 것
- 하향식 통합 테스트의 경우 넓이 우선 방식으로 테스트를 할 모듈을 선택할 수 있따.
- 상향 : 조빱에서 위로
- 모듈 간의 인터페이스와 시스템의 동작이 정상적으로 잘되고 있는지를 빨리 파악하고자 할때 상향식 보다는 하향식이 좋다
    
Json
: 인터페이스 구현 시 사용하는 기술로 속성-값 쌍으로 이루어진 데이터 오브젝트를 전달하는 포멧
...
ER모델
![image](https://github.com/pointmina/EIP/assets/68779817/4d4f0497-9f93-46fa-9a26-dce11d31566d)

로킹
: 데베 병행 제어를 위해 트랜잭션이 접든하고자 하는 데이터를 잠가 다른 트랜잭션이 접든하지 못하도록 하는 병행 제어 기법
- 데이터베이스, 파일, 레코드 등은 로킹 단위가 될 수 있따.
- 로킹 단위가 작아지면 데이터베이스 공유도가 증가한다.
- 한꺼번에 로킹 할 수 있는 객체의 크기를 로킹 단위라고 한다.
- 로킹 단위가 커지면 오버헤드가 감소한다
- 
![image](https://github.com/pointmina/EIP/assets/68779817/9dbcd1ef-419f-4da5-b77f-b02b9e49ce1c)
...
운영체제
- 다중 사용자와 다중 응용프로그램 환경하에서 자원의 현재 상태를 파악하고 자원 분배를 위한 스케쥴링을 담당한다
- CPU, 메모리 공간, 기억 장치, 입출력 장치 등의 자원을 관리한다
- 운영체제의 종류로는 맥, 리눅스, 안드로이드, 윈도우 드읃ㅇ
- 입출력 장치와 사용자 프로그램을 제어한다.

배치 프로그램의 필수요소
1. 대용량 데이터 : 대용량의 데이터를 처리할 수 있어야 한다.
2. 자동화 : 심각한 오류 상황 외에는 사용자의 개입 없이 동작해야 한다.
3. 견고성 : 유효하지 않은 데이터의 경우도 처리해서 비정상적인 동작 중단이 발생하지 않아야한다.
4. 안정성 : 어떤 문제가 생겼는지, 언제 발생했는지 등을 추적할 수 있어야한다.
5. 성능 : 주어진 시간 내에 처리를 할 수 있어야하고, 동시에 동작하고 있는 다른 앱을 방해하지 말아야한다. 

결합도
- 소프츠웨어 구조 내에서 모듈간의 관련성을 측정하는 척도
- 결합도는 모듈 내부가 아닌 외부의 모듈과의 연관도 또는 모듈간의 상호의존성을 나타냄
- 결합도가 낮을수록 좋다
- 내공외제스자 순으로 강하다
- 내가 공부 왜해야하는데 재수자할끄야
- 두 모듈간의 상호작용 또는 의존도 정도를 나타내는 것
내용(content) : 하나의 모듈이 직접적으로 다른 모듈의 내용을 참조할 때 
공통(common) : 두 모듈이 동일한 전역 데이터를 접근
외부(external)
제어(control)
스탬프(stamp) : 자료구조 형태로 전달되어 이용될 때
자료(data) : 매개변수나 인수를 넘겨주고, 호출 받은 모듈은 받은 데이터에 대한 처리 결과를 다시 돌려주는 방식

응집도
: 하나의 모듈이 하나의 기능을 수행하는 요소들간의 연관성 척도, 독립적인 모듈이 되기 위해서는 응집도가 약해야한다. 
- 위에가 젤 강함
기능적(functional)
순차적(sequential)
교환적/통신적(communication)
절차적(procedural)
시간적(temporal)
논리적(logical)
우연적(coincidental)
...
쉘
: 시스템과 사용자 간의 인터페이스 담당, 명령어 해석기
- 사용자 명령을 해석하고 커널로 전달하는 기능 제공
- 반복적인 명령 프로그램을 만드는 프로그래밍 기능을 제공한다
- 초기화 파일을 이용해 사용자 환경을 설정하는 기능을 제공한다.

OSI 7 계층
- 전송 계층 : 단말기 사의 오류 수정과 흐름제어를 수행하여 신뢰성 있고 명확한 데이터 전달
- 응용 계층 : 최종 사용자가 실제로 상호 작용하는 계층, 네트워크 리소스에 대한 액세스를 허용
- 세션 계층 : 컴퓨터 간의 세션을 설정, 관리 및 종료하여 적절한 통신을 유지하는 역할
- 표현 계층 : 데이터로 작동하는 계층, 주요 기능은 데이터 변환, 암호화 및 압축 

TCP 흐름제어기법
- Slow Start : 1씩 증가 1씩 감소
- Sliding Window : 한번에 여러 패킷 전송
- Stop and Wait : 손실시 1개 전송 응답 기다림
- Congestion Avoidance : 패킷의 트래픽이 붕괴되지 않도록 흐름을 제어하는 트래픽 제어

UDP(User Datagram Protocol)
- 비연결성 서비스 제공
- 단순한 헤더구조로 오버헤드 적음
- 빠른 속도로 전송 요구시, 동시에 여러 사용자에게 데이터
- 제어가 없어서 신뢰성이 낮다
...
- IGMP : 멀티캐스트를 지원하는 호스트나 라우터 사이어서 멀티캐스트 그룹유지를 위해 사용
...
프로세스
- 디스패치 : 프로세스가 준비 상태에서 프로세스가 배당되어 실행 상태로 변화하는 것
- 프로세스 제어 블록(PCB) : 프로세스 식별자, 프로세스 상태 등의 정보로 구성된다
...
비용 산정 기법
- 전문가 감정 기법 : 조직 내에 있는 경험 많은 두명 이상의 전문가에게 비용 산정 의뢰
- 델파이 기법 : 전문가 감정 기법의 주관적 편견을 보완하기 위해 많은 전문가의 의견을 종합
- LOC 기법 : 원시 코드 라인 수의 비관치 낙관치 기대치를 측정하여 산정하는 기법
- 개발 단계별 인원수 기법 : LOC 보완, 생명 주기를 각 단계별로 선정
- COCOMO : 보헴, LOC 비용 산정 기법

비용 산정
- Putnam : 소프트웨어 생명 주기의 전 과정 동안에 사용될 곡선의 노력의 분포를 가정해주는 모형, Rayleigh-Norden 곡선의 노력 분포를 기초로
- FP기법 : 기능 점수 모형으로 알브레히트가 제안
